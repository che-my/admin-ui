...
<template>
  <div v-if="['side', 'head', 'single'].includes(settingsState.menu.menuMode) || settingsState.mode === 'mobile'" class="sub-sidebar-container" :class="{'is-collapse': settingsState.mode === 'pc' && settingsState.menu.subMenuCollapse}" @scroll="onSidebarScroll">
    <Logo
      :show-logo="settingsState.menu.menuMode === 'single'" :class="{
        'sidebar-logo': true,
        'sidebar-logo-bg': settingsState.menu.menuMode === 'single',
        'shadow': sidebarScrollTop
      }"
    />
    <!-- 侧边栏模式（无主导航） -->
    <el-menu
      :unique-opened="settingsState.menu.subMenuUniqueOpened" :default-openeds="menu.defaultOpenedPaths()" :default-active="route.meta.activeMenu||route.path" :collapse="settingsState.mode === 'pc' && settingsState.menu.subMenuCollapse" :collapse-transition="false" :class="{
        'is-collapse-without-logo': settingsState.menu.menuMode !== 'single' && settingsState.menu.subMenuCollapse
      }"
    >
